...
// Copyright (C) 2012  Miroslav Spousta
//
...
//
#define        COPYSTRINGtoWORD(WORD,STR,LEN)           \
{                                                       \
    do                                                  \
    {                                                   \
        WORD = (Word_t)(STR)[0] << 56;                  \
        if (LEN == 1) break;                            \
        WORD += ((Word_t)(STR)[1] << 48);               \
        if (LEN == 2) break;                            \
        WORD += ((Word_t)(STR)[2] << 40);               \
        if (LEN == 3) break;                            \
        WORD += ((Word_t)(STR)[3] << 32);               \
        if (LEN == 4) break;                            \
        WORD += ((Word_t)(STR)[4] << 24);               \
        if (LEN == 5) break;                            \
        WORD += ((Word_t)(STR)[5] << 16);               \
        if (LEN == 6) break;                            \
        WORD += ((Word_t)(STR)[6] << 8);		\
	if (LEN == 7) break;				\
        WORD += ((Word_t)(STR)[7]);			\
...
//
#define         COPYWORDtoSTRING(STR,WORD,LEN)          \
{                                                       \
    do                                                  \
    {                                                   \
        (STR)[0] = (uint8_t)((WORD) >> 56);		\
	if (LEN == 1) break;				\
        (STR)[1] = (uint8_t)((WORD) >> 48);       	\
	if (LEN == 2) break;				\
        (STR)[2] = (uint8_t)((WORD) >> 40);       	\
	if (LEN == 3) break;				\
        (STR)[3] = (uint8_t)((WORD) >> 32);       	\
	if (LEN == 4) break;				\
        (STR)[4] = (uint8_t)((WORD) >> 24);       	\
	if (LEN == 5) break;				\
        (STR)[5] = (uint8_t)((WORD) >> 16);       	\
	if (LEN == 6) break;				\
        (STR)[6] = (uint8_t)((WORD) >>  8);       	\
	if (LEN == 7) break;				\
        (STR)[7] = (uint8_t)(WORD);                     \
    } while(0);                                         \
}
...
{                                                       \
    do                                                  \
    {                                                   \
        (STR)[0] = (uint8_t)((WORD) >> 24);       	\
	if (LEN == 1) break;				\
        (STR)[1] = (uint8_t)((WORD) >> 16);       	\
	if (LEN == 2) break;				\
        (STR)[2] = (uint8_t)((WORD) >>  8);       	\
	if (LEN == 3) break;				\
        (STR)[3] = (uint8_t)(WORD);			\
    } while(0);                                         \
}
...
//
#define MEMCMP(S1,S2,N)   memcmp((void *)(S1), (void *)(S2), (N))
#define MEMCPY(S1,S2,N)   memcpy((void *)(S1), (void *)(S2), (N))
...
#define SCLCMP(INDEX,PSCL,LEN) MEMCMP(INDEX, PSCLINDEX(PSCL), LEN)

#define PPSCLVALUE_EQ(INDEX,PSCL,LEN)                                       \
    ((SCLCMP(INDEX, PSCL, LEN) == 0) ? &PSCLVALUE(PSCL) : (PPvoid_t)NULL)

#define PPSCLVALUE_LT(INDEX,PSCL,LEN)                                       \
    ((SCLCMP(INDEX, PSCL, LEN) < 0) ? &PSCLVALUE(PSCL) : (PPvoid_t)NULL)

#define PPSCLVALUE_GT(INDEX,PSCL,LEN)                                       \
    ((SCLCMP(INDEX, PSCL,LEN) > 0) ? &PSCLVALUE(PSCL) : (PPvoid_t)NULL)
...
{                                                                       \
    ...
    *(PPARRAY) = (Pvoid_t)SET_PSCL(PSCL);                               \
    ...
    (void)MEMCPY((PSCL)->scl_Index, INDEX, LEN);                       \
}
...
PPvoid_t
JudySLFKGet(Pcvoid_t PArray, const uint8_t * Index, Word_t len, PJError_t PJError)
{
...
// Upon invalid Index or end of Index (string) in current word, return.
    ...
    do                                  // until return.
    {
        if (IS_PSCL(PArray))            // a shortcut leaf.
            return (PPSCLVALUE_EQ(pos, PArray, len));

        COPYSTRINGtoWORD(indexword, pos, len);       // copy next 4[8] bytes.

        JLG(PPValue, PArray, indexword);

        if ((PPValue == (PPvoid_t) NULL) || len <= WORDSIZE)
            return (PPValue);
...
PPvoid_t
JudySLFKIns(PPvoid_t PPArray, const uint8_t * Index, Word_t len, PJError_t PJError)
{
...
//          first check if string is already inserted
            ...
            if (MEMCMP(pos, pos2, len) == 0)
                return (&(Pscl->scl_Pvalue));

            *PPArray = (Pvoid_t)NULL;   // disconnect SCL.

            scl2 = SCLSIZE(len);       // save for JudyFree
...
// See header comments about premature return().
        ...
        if (Pscl != (Pscl_t) NULL)
        {
            COPYSTRINGtoWORD(indexword2, pos2, len); // copy next 4[8] bytes.
...
int
JudySLFKDel(PPvoid_t PPArray, const uint8_t * Index, Word_t len, PJError_t PJError)   // optional, for returning error info.
{
...
// Do the deletion:
    ...
    return (JudySLDelSub(PPArray, PPArray, Index, len, PJError));
...
// for other Indexes are not coalesced.
    ...
    if (IS_PSCL(*PPArray))
    {
        ...
        if (MEMCMP(Index, Pscll->scl_Index, len))
...
// the current JudyL array, and return the result all the way up.
    ...
    COPYSTRINGtoWORD(indexword, Index, len); // copy next 4[8] bytes.
...
PPvoid_t
JudySLFKPrev(Pcvoid_t PArray, uint8_t * Index, Word_t len, PJError_t PJError) // optional, for returning error info.
{
...
// SCLs index; otherwise return null.
    if (orig)
    {
        if (IS_PSCL(PArray))
        {
            if ((PPValue = PPSCLVALUE_GT(Index, PArray, len)) != (PPvoid_t) NULL)
                (void)MEMCPY(Index, PSCLINDEX(PArray), len);
            return (PPValue);
        }
...
// words).
    ...
    COPYWORDtoSTRING(Index, indexword, len); // copy next 4[8] bytes.
    if (len <= WORDSIZE)
        return (PPValue);
// If a previous JudySLIns() ran out of memory partway down the tree, it left a
// null *PPValue; this is automatically treated as a dead-end (not a core dump
...
PPvoid_t
JudySLFKNext(Pcvoid_t PArray, uint8_t * Index, Word_t len, PJError_t PJError) // optional, for returning error info.
{
// Check for caller error (null pointer), or empty JudySL array:
              ...
              PJError_t PJError)        // optional, for returning error info.
{
    ...
    if (orig)
    {
        if (IS_PSCL(PArray))
        {
            if ((PPValue = PPSCLVALUE_LT(Index, PArray, len)) != (PPvoid_t) NULL)
                (void)MEMCPY(Index, PSCLINDEX(PArray), len);
            return (PPValue);
        }
...
PPvoid_t
JudySLFKFirst(Pcvoid_t PArray, uint8_t * Index, Word_t len, PJError_t PJError)        // optional, for returning error info.
{
    PPvoid_t  PPValue;                  // from JudyL array.
    if (Index == (uint8_t *) NULL)
    {
    ...
    if ((PPValue = JudySLFKGet(PArray, Index, len, PJError)) == PPJERR)
        return (PPJERR);                // propagate serious error.
    if ((PPValue == (PPvoid_t) NULL)    // first try failed.
        && ((PPValue = JudySLFKNext(PArray, Index, len, PJError)) == PPJERR))
    {
        return (PPJERR);                // propagate serious error.
    }
...
PPvoid_t
JudySLFKLast(Pcvoid_t PArray, uint8_t * Index, Word_t len, PJError_t PJError) // optional, for returning error info.
{
    PPvoid_t  PPValue;                  // from JudyL array.
    if (Index == (uint8_t *) NULL)
    {
    ...
    if ((PPValue = JudySLFKGet(PArray, Index, len, PJError)) == PPJERR)
        return (PPJERR);                // propagate serious error.
    if ((PPValue == (PPvoid_t) NULL)    // first try failed.
        && ((PPValue = JudySLFKPrev(PArray, Index, len, PJError)) == PPJERR))
    {
        return (PPJERR);                // propagate serious error.
    }
...
Word_t
JudySLFKFreeArray(PPvoid_t PPArray, Word_t len, PJError_t PJError)    // optional, for returning error info.
{
    PPvoid_t  PPArrayOrig = PPArray;    // for error reporting.
...
// (null-padded) word is null.
         ...
         PPValue = JudyLNext(*PPArray, &indexword, PJError))
    {
        if (len > WORDSIZE)
        {
            if ((bytes_freed = JudySLFKFreeArray(PPValue, len-WORDSIZE, PJError)) == JERR)
